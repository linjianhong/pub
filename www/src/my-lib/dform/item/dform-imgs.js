(function () {
    ...
    .component('dformImgs', {
      template: `
        ...
        <div class="flex flex-between" ng-if="mode!='show' && selectIndex>=0">
          <div class="imgs-edit-btn" ng-click="edit_cut()"><i class="fa fa-cut"></i></div>
          <div class="imgs-edit-btn" ng-click="edit_left()"><i class="fa fa-arrow-left"></i></div>
          <div class="imgs-edit-btn" ng-click="edit_right()"><i class="fa fa-arrow-right"></i></div>
          <div class="imgs-edit-btn" ng-click="edit_exit()"><i class="fa fa-undo"></i></div>
        </div>`,
      bindings: {
        maxCount: "<",
        imgs: "<",
  ...
  function ctrl($scope, $http, $element, IMG, DjPop) {
    ...
    $scope.edit_left = () => {
      ...
      $scope.$emit("input.component.valueChange", { value: $scope.imgList });
    }
    $scope.edit_right = () => {
    ...
    this.deleteImg = (n, imgs) => {
      ...
      return DjPop.confirm("您确认要删除当前图片?").then(a => {
        ...
        $scope.$emit("input.component.valueChange", { value: imgs });
      })
    }
    $scope.clickImg = (n) => {
