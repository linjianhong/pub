  ...
  angular.module("dj.router.frame").component("quickMenu", {
    template: `
      <div class="icon-cells-box" ng-if="otherPower[otherPowerName].length" ng-repeat="otherPowerName in otherPowerNames">
        <div class="icon-cell-caption text"><i class="fa fa-{{otherPowerFa[otherPowerName]}} text-green em-15"></i>　{{otherPowerName}}</div>
        <div class="icon-cells flex-left flex-wrap">
          <div ng-click="clickBtn(btn)" class="flex-v flex-cc icon-cell" ng-repeat="btn in otherPower[otherPowerName]" ng-style="{color: btn.color||'#008'}">
