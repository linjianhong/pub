...
!(function (angular, window, undefined) {
  angular.module("dj-view").component("addressSelect", {
    template: `<div class="flex bk-f padding-3 {{$ctrl.args.css||''}}" ng-click="addminAddress()">
      <div class="flex-cc address-icon"><i class="fa fa-map-marker"></i></div>
      <div class="flex-1 flex-v padding-h-3">
        <div class="flex-left"><div class="em-15">{{reciever.name}}</div><div class="text-8 padding-h-3">{{reciever.mobile}}</div></div>
        <div>{{reciever.location}} {{reciever.detail}}</div>
        <div ng-if="$ctrl.address && !reciever">点击创建收货地址...</div>
      </div>
      <div class="em-15" ng-if="$ctrl.mode!='show'"><i class="fa fa-angle-right text-c"></i></div>
    </div>`,
    bindings: {
      args: "<",
      onChange: "&",
    ...
    controller: ["$scope", "$http", "DjState", "DjRouter", function ctrl($scope, $http, DjState, DjRouter) {
      ...
      this.$onChanges = (changes) => {
        if (changes.args && changes.args.currentValue) {
          var args = changes.args.currentValue;
      ...
      var autoReciever = () => {
        ...
        $http.post("buyer/address").then(json => {
          var list = $scope.list = (json.datas.list || [])//.filter(item => item.type == "收货地址");
          $scope.reciever = list.find(item => item.main) || list[0];
          console.log("收货地址", list);
        });
      }
      autoReciever();
      ...
      $scope.addminAddress = () => {
        ...
        return $http.post("dform-dialog-component", {
          component: "dlg-addmin-address",
          params: {
            param: {
              list: $scope.list,
            },
          ...
          console.log("选择", data);
          var $value = data || $scope.reciever;
          $scope.reciever = $value
        });
      }

      $scope.$watch("reciever", (vNew, vOld) => {
        if (!vNew && !vOld || angular.equals(vNew, vOld)) return;
        this.onChange && this.onChange({ $value: vNew });
      })
    }]
  });

  angular.module("dj-view").component("dlgAddminAddress", {
    template: `
    <div class="flex-v bk-f fixed-page-box">
      <div class="flex bb-ccc">
        <div class="padding-2 flex-cc text-8 em-20" ng-click="cancel()"><i class="fa fa-angle-left"></i></div>
        <div class="padding-2 flex-cc text-active em-20" ng-click="addAddress()"><i class="fa fa-plus"></i></div>
      </div>
      <div class="flex-1 flex-v v-scroll">
        <div class="flex padding-h-2 bb-ccc" ng-repeat="item in $ctrl.param.list track by $index">
          <div class="padding-v-2 em-12">
            <div class="text-c address-first-name flex-cc">{{item.name.length==2&&item.name||item.name.substr(0,1)}}</div>
          </div>
          <div class="flex-1 padding-2 b-900" ng-click="selectAddress(item)">
            <div class="flex-left">
              <div class="text-3 flex em-15">{{item.name}}</div>
              <div class="text-8 padding-h-3 flex-1">{{item.mobile}}</div>
              <div class="address-main-mark em-12" ng-if="item.main">默认</div>
            </div>
            <div class="text-8 em-12">{{item.location}} {{item.detail}}</div>
          </div>
          <div class="text-8 bl-ccc padding-2" ng-click="editAddress(item)">编辑</div>
        </div>
      </div>
    </div>`,
    bindings: {
    ...
    controller: ["$scope", "$http", "DjState", "DjRouter", function ctrl($scope, $http, DjState, DjRouter) {
      ...
      this.$onChanges = (changes) => {
        if (changes.param && changes.param.currentValue) {
          var param = changes.param.currentValue;
          if (param.list && !param.list.length) {
            setTimeout($scope.addAddress);
          }
        }
      }
      ...
      $scope.selectAddress = (item) => {
        $scope.$emit("dform-pop-box-close", { btnName: "OK", value: item });
      }
      $scope.editAddress = (item) => {
        editAddress(item).then(value => {
          value = angular.extend({}, value);
          angular.extend(item, value);
          console.log("value", value);
          if (value.main) {
            this.param.list.map(a => a !== item && (delete a.main))
          }
          console.log("更新", this.param.list)
          $http.post("buyer/update_address", { value: this.param.list }).then(json => {
          });
        });
      }
      $scope.addAddress = () => {
        editAddress({}).then(value => {
          value = angular.extend({}, value);
          console.log("value", value);
          $http.post("buyer/add_address", { value }).then(json => {
            this.param.list.push(value);
          });

        });
      }
      function editAddress(initValue) {
        editAddress.form = {
          items: $scope.items,
          initValue,
        };
        return $http.post("dform-dialog-form", {
          params: {
            "class": "bk-f fixed-page-box",
            param: {
              title: "添加收货地址",
              form: editAddress.form,
            },
          },
          options: { scope: $scope },
        });
      ...
      /** 自动定位时的详细地址 */
      $scope.$on("address-location-detail", (event, data) => {
        event.preventDefault();
        event.stopPropagation();
        editAddress.form.initValue2 = { detail: data };
      });

      $scope.items = [
        { name: "name", title: "收货人", valid: { required: 1 }, invalid: { required: "请输入" } },
        { name: "mobile", title: "手机号码", valid: { required: 1, pattern: /^1(\d|\d-){10}$/ }, invalid: { required: "请输入", pattern: '手机号码不合法', } },
        { name: "location", title: "收货地址", component: "address-location", valid: { required: 1, minlength: 5 }, invalid: { required: "请输入", minlength: "至少5个字" } },
        { name: "detail", title: "详细地址", type: "input", valid: { required: 1, minlength: 3 }, invalid: { required: "请输入", minlength: "至少3个字" } },
        { name: "main", title: "设为默认", type: "yn" },
      ];
    }]
  });
   ...
   * 可自动定位到当前位置
   */
  angular.module('dform').component('addressLocation', {
    template: `
    <div class="flex">
      <div class="padding-1 flex-1 {{text&&'text-6'||'text-c'}}" ng-click="clickText()">{{text||'请点击选择地址'}}</div>
      <div class="padding-h-2 em-15 text-8" ng-click="here()"><i class="fa fa-map-marker"></i></div>
    </div>`,
    bindings: {
      tags: "<",
      config: "<",
    ...
    controller: ["$scope", "$http", "$q", function ctrl($scope, $http, $q) {
      ...
      $scope.$on("input.component.initValue", (event, data) => {
        console.log("input.component.initValue", data);
        $scope.text = data.value;
      });

      $scope.here = () => {
        $http.post("地址信息").then(arr => {
          console.log("地址信息", arr);
          var detail = arr.pop();
          $scope.text = arr.join(" ");
          submitValue();
          $scope.$emit("address-location-detail", detail);
        }).catch(e => {
          console.error("地址信息", e)
        });
      }

      $scope.clickText = () => {
        console.log("要修改。。。")
        editAddress($scope.text).then(text => {
      ...
      function editAddress(initValue) {
        return $http.post("dform-dialog-component", {
          component: "dlg-address-district",
          params: {
            "class": "bk-f fixed-bottom-80-box",
            text: initValue,
          },
          options: {},
        });
      ...
      function submitValue() {
        $scope.$emit("input.component.valueChange", { value: $scope.text });
      }
  ...
  /** 省市区选择组件 的 对话框 */
  angular.module('dform').component('dlgAddressDistrict', {
    template: `
    <div class="flex bb-ccc">
      <div class="padding-2 flex-1">请选择</div>
      <div class="padding-2 text-8" ng-click="cancel()"><i class="fa fa-close"></i></div>
    </div>
    <div class="flex-left bb-ccc">
      <div class="padding-3 {{$index==R.selecting&&'text-warning'||'text-3'}}"
        ng-click="R.setSelect($index)"
        ng-repeat="text in R.arr track by $index"
      >{{text}}</div>
      <div class="padding-3 text-8" ng-if="!R.arr.length">请选择</div>
    </div>
    <div class="padding-h-2">
      <div class="text-c">{{R.selectPrompt[R.selecting]}}</div>
      <div class="flex-wrap">
        <div class="padding-3 {{text==R.arr[R.selecting]&&'text-warning'||'text-3'}}"
    ...
    controller: ["$scope", "$http", "$q", function ctrl($scope, $http, $q) {
      ...
      var R = $scope.R = {
        arr: [],
        selecting: 0,
        selectPrompt: ["选择省份/地区", "选择城市", "选择县区"],
        setSelect: index => {
          if (index < 0) index = 0
          R.selecting = index;
          R.list = GetList(index, R.arr, CHINA_AREAS);
        },
        select: text => {
          R.arr[R.selecting] = text;
          R.selecting++;
          R.arr.length = R.selecting;
          R.list = GetList(R.selecting, R.arr, CHINA_AREAS);
          console.log("R=", R);
          if (R.list) R.arr.push(R.selectPrompt[R.selecting]);
          else {
            $scope.$emit("dform-pop-box-close", { btnName: "OK", value: R.arr.join(" ") });
          }
        },
      };
      ...
      this.$onChanges = (changes) => {
        if (changes.text) {
          init(changes.text.currentValue);
        }
      }

      function init(text) {
        R.arr = text && angular.isString(text) && text.split(" ") || [R.selectPrompt[0]];
        R.setSelect(R.arr.length - 1);
      }
  ...
  function GetList(nth, arr, areas) {
    if (nth <= 0) {
      if (angular.isArray(areas)) return areas;
      if (angular.isObject(areas)) return Object.keys(areas);
      return false;
    }
    areas = areas[arr[0]];
    arr = arr.filter((v, n) => n);
    return GetList(nth - 1, arr, areas)
  }
  ...
  var CHINA_AREAS = {
    ...
    "青海省": {
      ...
      "海西蒙古族藏族自治州": ["格尔木市", "德令哈市", "乌兰县", "都兰县", "天峻县", "海西蒙古族藏族自治州直辖"],
    },
    "宁夏回族自治区": {
