!(function (window, angular, undefined) {
  ...
  angular.module('dj-ui').factory("IMG", ["$http", "$q", function ($http, $q) {
    ...
    function preview(active, imgs) {
      imgs = imgs.filter(img => fileType(img) == "img");
      if (!imgs.length) return;
      $http.post("微信预览图片", { active, imgs }).catch(e => {
        $http.post("显示对话框/gallery", { params: { imgs, active } });
      });
    }
    ...
    function compressImgToDataURL(img, orientation) {
      ...
      //如果图片大于8千万像素, 计算压缩比并将大小压至8千万以下
      var ratio = width * height / 8e8;
      if (ratio > 1) {
  ...
  angular.module('dj-ui').directive("dformFilesUpload", function () {
    return {
