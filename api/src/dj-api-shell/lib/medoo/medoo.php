...
class medoo
{
	...
	protected function column_push(&$columns)
	{
		...
		foreach ($columns as $key => $value)
		{
			...
			{
  			//Medoo 不够强大，只能自己添加一些SQL支持了：：
  			preg_match('/^(sum|count|max|min|distinct)\(/i', $value, $match);
  			if (isset($match[1]))
  			{//添加SQL函数原生支持：
	...
	protected function where_clause($where)
	{
		...
		if (is_array($where))
		{
			...
			if (isset($where[ 'GROUP' ]))
			{
				// ----------------------------------
				// added by LinJH
				// GROUP BY can use array
				$GROUP = $where['GROUP'];
				if (is_array($GROUP)) {
					$values = [];
					foreach ($GROUP as $column => $value) {
						$values[] = $this->column_quote($value);
					}
					$where_clause .= ' GROUP BY ' . implode(",", $values);
				} else
				// the end of add.  ----------------------
